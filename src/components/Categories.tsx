  ...
  return <section id="categories" className="py-20 md:py-32 bg-white/50 relative overflow-hidden">
      ...
      <div className="container max-w-7xl mx-auto px-4 sm:px-6 relative">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 md:gap-8">
          ...
          return <AnimatedElement key={category.id} animation="fade-in" delay={index * 100 + 600} className="group">
                <Link to={`/category/${category.id}`} className="h-full">
                  <div className="glass-card h-full flex flex-col items-center justify-center p-6 text-center transition-all duration-300 group-hover:shadow-soft-lg">
                    {hasImage ? <div className="mb-4 p-2 rounded-full bg-white/80 transform transition-transform duration-500 group-hover:scale-110">
                        <img src={categoryImages[category.id]} alt={`${category.name} image`} className="w-20 h-20 object-contain" />
                      </div> : <div className="mb-4 p-4 rounded-full bg-primary/10 text-primary transform transition-transform duration-500 group-hover:scale-110 group-hover:bg-primary/20">
                        <Icon size={32} />
                      </div>}
        ...
        <AnimatedElement animation="fade-in" delay={1100} className="mt-16 text-center">
          <Link to="/products" className="btn-secondary inline-flex items-center gap-2 px-8 py-3 hover:shadow-soft">
            View All Products
          </Link>
        </AnimatedElement>
      </div>
